<!-- Header -->
<header id="home">
	<!-- Background Image -->
	<!-- /Background Image -->

	<!-- Nav -->
	<nav id="nav" class="navbar nav-transparent">
		<div class="container">

			<div class="navbar-header">
				<!-- Logo -->
				<div class="navbar-brand">
					<img class="logo-alt" src="data/img/Header-Logo.png" alt="logo">
					<img class="logo" src="data/img/Header-Logo.png" alt="logo">
				</div>
				<!-- /Logo -->

				<!-- Collapse nav button -->
				<div class="nav-collapse">
					<span></span>
				</div>
				<!-- /Collapse nav button -->
			</div>

			<!--  Main navigation  -->
			<ul class="main-nav nav">
				<li><a href="#home"><img src="data/img/icons/home.png"></img>Home</a></li>
				<li><a href="#about"><img src="data/img/icons/about us.png">About</a></li>
				<li><a href="#service"><img src="data/img/icons/services.png">Services</a></li>
				<li><a href="#portfolio"><img src="data/img/icons/gallery.png">Gallery</a></li>
				<li><a href="#team"><img src="data/img/icons/our team.png">Affiliates</a></li>
				<li><a href="#blog"><img src="data/img/icons/whats up.png"/>News</a></li>
				<li><a href="#contact"><img src="data/img/icons/contact us.png">Contact</a></li>
			</ul>
			<!-- /Main navigation -->

		</div>
	</nav>
	<!-- /Nav -->

</header>
<!-- /Header -->